    ...
    for (procedure_number in sort(oeil_searches)){
      ...
      if (connection_established==TRUE){
        if (TRUE) { ### Following loop allows me to deactivate scraping in case of debugging to save time
          ...
          if(!is_empty(which(start_procedure_labels %in% key_events_event))){
            for (lbl in start_procedure_labels[which(start_procedure_labels %in% key_events_event)]){ # Loop over labels used to identify start of procedure and generate string to path
...
cat('Citation:\nScotto di Vettimo, M. (2022), "scRapEU: An R function to scrape data on EU laws". Version July 2024. DOI: 10.5281/zenodo.10871232.\n\n')
cat('NOTE: The function is still under active development! Comments and suggestions are welcome.\n')
cat('Last update: 4 July 2024.\n\n')