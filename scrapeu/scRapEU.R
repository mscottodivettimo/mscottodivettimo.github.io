...
scRapEU <- function(exact_procedures = NULL, years, procedures = NULL, verbose =  FALSE){
  ...
  KEY_EVENTS <- function() {
    ...
    trilogue <- ifelse(any(key_events_event %in% EVENT_LABELS$trilogue_labels), 1, 0)
  ...
  DOCUMENT_GATEWAY <- function() {
    ...
    date_initiation <- key_events_date[which(key_events_event %in% EVENT_LABELS$start_procedure_labels)][1]
    
    if(is_empty(date_initiation) | is.na(date_initiation)){ date_initiation <- docgate_date[which(docgate_documents %in% EVENT_LABELS$start_procedure_labels)][1]} 
    ...
    revised_proposal <- ifelse(any(key_events_event %in% EVENT_LABELS$revision_proposal_labels) == TRUE, 1, 0)
    revised_proposal <- ifelse(length(key_events_date[which(key_events_event %in% EVENT_LABELS$start_procedure_labels)]) > 1, 1, revised_proposal)
    
    ### Revised proposal date and number
    if(revised_proposal == 1){
      revised_proposal_date <- key_events_date[which(key_events_event %in% EVENT_LABELS$revision_proposal_labels)]
      revised_proposal_number <- key_events_documents[which(key_events_event %in% EVENT_LABELS$revision_proposal_labels)]
    } else { revised_proposal_date <- NA; revised_proposal_number <- NA }
    
    ### Procedure stage if missing info on page
    if(!exists("stage_reached")){ 
      if(any(EVENT_LABELS$end_procedure_labels %in% key_events_event)){
    ...
    return(list(
      ...
      docgate_documents = docgate_documents
    ))
    ...
    if(!is_empty(proposal_number)){
      if(any(proposal_number %in% unlist(str_split(table_rows,'\t')))){
        ...
        if(is_empty(proposal_links)){
          ...
          remDr$navigate(paste0('https://eur-lex.europa.eu/search.html?text=%22',proposal_number,'%22&lang=en&type=quick&qid=1763491671080&scope=EURLEX'))
          ...
          if (!inherits(attempt_res, "try-error")) {
            ...
            # Should check before assigning the following link to proposal 
    ...
    if(any(grepl('https://eur-lex.europa', proposal_links, fixed = TRUE))){
      ...
      if (connection_eurlex_proposal_established){
        ...
        if(!is.na(attempt_celex_proposal)) {
          ...
          if (stringdist::stringsim(str_remove_all(str_remove_all(attempt_celex_proposal,'^5'),'\\D'), 
                                    str_remove_all(proposal_number,'\\D'),
                                    method = 'cosine') > .95) {
  ...
  NON_PENDING_AND_COMPLETE <- function() {
    ...
    if(all(!is.na(key_events_event)) & !is_empty(key_events_event)) {
      date_end <- unique(key_events_date[which(key_events_event %in% EVENT_LABELS$end_procedure_labels)])
      if(is_empty(date_end)){ date_end <- key_events_date[length(key_events_date)] }
      if(length(date_end) > 1){ date_end <- unique(date_end[which(as.Date(date_end, format = '%d/%m/%Y') == max(as.Date(date_end, format = '%d/%m/%Y')))]) }
      if((!is.na(date_initiation) & !is.na(date_end))) {
        if(as.Date(date_end, format = '%d/%m/%Y') < as.Date(date_initiation, format = '%d/%m/%Y')) {
          date_end <- unique(key_events_date[which(as.Date(key_events_date, format = '%d/%m/%Y') == max(as.Date(key_events_date, format = '%d/%m/%Y')))])
        }
        # Check again that the date_end makes sense
        if(as.Date(date_end, format = '%d/%m/%Y') < as.Date(date_initiation, format = '%d/%m/%Y')) {
          date_end <- NA
        }
      }
    ...
    if(is.na(date_end)) {
      if(!is.null(docgate_documents)) {
        date_end <- unique(docgate_date[which(docgate_documents %in% EVENT_LABELS$end_procedure_labels)])
        if((is_empty(date_end) | length(date_end) > 1)) {
          date_end <- unique(docgate_date[which(as.Date(docgate_date, format = '%d/%m/%Y') == max(as.Date(docgate_date, format = '%d/%m/%Y')))])
        }
      }
    }
    ...
    if (procedure_status == 'Completed'){
      ...
      if(is_empty(url_eurlex_final) & !is.na(proposal_ALL)){
      ...
      if(!is_empty(url_eurlex_final)){
        ...
        } # End of condition if connection with EUR-Lex final is established
  ...
  suppressWarnings(EVENT_LABELS <- readLines('https://www.dropbox.com/scl/fi/dmnllqke6yl0s81ceuu44/key_events_list.txt?rlkey=z1a0obqy1xncg484tmyojrr89&st=97uyu2m2&dl=1'))
  eval(parse(text = EVENT_LABELS))
  ...
  for (year in as.character(c(years))){ # Loop over the years
    ...
    for (procedure_number in sort(oeil_searches)){ procedure_number <<- procedure_number
      ...
      rm(list = setdiff(ls(),c('df','variables','year','i','date_scraped','%!in%','remDr','remote_driver',
                               ...
                               "OEIL_TECHNICAL_INFORMATION",'KEY_PLAYERS','KEY_EVENTS',"DOCUMENT_GATEWAY","PROPOSAL_INFO_AND_CELEX",
                               'ACTIVATE_DRIVER', "NON_PENDING_AND_COMPLETE","EVENT_LABELS",
...
cat('Citation:\nScotto di Vettimo, M. (2022), "scRapEU: An R function to scrape data on EU laws". Version May 2026. https://michelescottodivettimo.eu/scrapeu/scRapEU.R.\n\n')
cat('NOTE: The function is continously updated. Suggestions, and reports of bugs are welcome.\n')
cat('Last update: 13 May 2026.\n\n')